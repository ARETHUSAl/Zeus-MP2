...
!
!  EXTERNALS:
!     empty -- macroname which is defined to be the user-supplied
!                subroutine name which re-initializes user-defined
!                variables for the problem to be restarted.
!                empty is undefined by default for backward 
...
!-------------------------  benchmark RESTART  ---------------------------
!
!  empty is a user defined cpp macroname representing a subroutine
!  which re-intializes all user-defined variables for the particular 
!  problem to be restarted.  empty should re-initialize only the
!  variables defined by the user in benchmark which are not in any
!  ZEUS-2D common blocks and are therefore not saved in the restart
!  dump.  empty must read the same namelist(s) as benchmark.
!
      call empty
!
!-------------------------  GRID MOTION CONTROL  -----------------------
