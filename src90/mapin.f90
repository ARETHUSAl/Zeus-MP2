...
!
!     PURPOSE: maps restart file arrays into ZEUS-MP arrays
!
...
!
!-----------------------------------------------------------------------
!     moving grid arrays -- 1 coordinate
!-----------------------------------------------------------------------
!
...
!
!-----------------------------------------------------------------------
!     moving grid arrays -- 2 coordinate
!-----------------------------------------------------------------------
!
...
!
!-----------------------------------------------------------------------
!     moving grid arrays -- 3 coordinate
!-----------------------------------------------------------------------
!
...
!
      if(xmhd) then
       call get_field_reals (b1,index)
       call get_field_reals (b2,index)
       call get_field_reals (b3,index)
      endif ! xmhd
!
!
      if(xhse) then
       call get_field_reals (gp1,index)
       call get_field_reals (gp2,index)
       call get_field_reals (gp3,index)
      endif ! hse
!
      if(lrad .ne. 0) then
       do k = 1, kn
...
!      dtusr   = rlrtvr(23)
      ...
      trem    = rlrtvr(39)
      tsave   = rlrtvr(40)
!JH   we don't store tused to ensure that restart files can be subjected
!JH   to a repeatability (diff) test.
      tused   = 0.0 ! rlrtvr(41)
      tusr    = rlrtvr(42)
      ttsl    = rlrtvr(43)
      v1floor = rlrtvr(44)
...
!
      hdffile = chrtvr(1:16)
      hstfile = chrtvr(17:32)
      resfile = chrtvr(33:48)
      usrfile = chrtvr(49:64)
      tslfile = chrtvr(65:73)
!
!=======================================================================
!     gravity.h arrays
!=======================================================================
