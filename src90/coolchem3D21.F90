...
!            are enforced every timestep by calling mconsrv.  Invoking
...
!
!            As noted earlier, ZEUS-MP cgs densities, times and energies 
...
!
      ...
      real(rl) :: qq,vibl,t_photo,t_heat,dtchem,t_chem,ci &
      ,           ttmin,cnorm,scoef,acoef,logtem0,dtimin &
      ,           logtem9,mhi,tphmin,thtmin,tsubcycle,R
#ifdef H &
      ,           HIp,HIIp,dep
#endif /* H */
#ifdef He &
...
#ifdef H2
      if (idust .eq. 1) then
        R     = 3.0d-17 * z_sol  ! H_2 formation rate coefficient 
                                 ! due to dust
      else
	R     = 0.
      endif
...
!      cnorm    = boltz*tevk/(1.22*mh)
      ...
      thtmin  = huge
      itmax   = 10000
      if (nhy .eq. 1 .and. irestart .ne. 1) then
      ceHItot   = 0.
      ciHItot   = 0.
      ceHeItot  = 0.
      ciHeItot  = 0.
...
!     Convert abundances to the mass densities needed to compute
!     number densities below.
      do k = ks, ke
       do j = js, je
        do i = is, ie
         dv_ioniz(i,j,k) = 0.
         do n  = 1, nspec
            abun(i,j,k,n) =  abun(i,j,k,n) * d(i,j,k)
         enddo
        enddo
       enddo
      enddo
...
!            if (tgas(i,j,k) .gt. 1.0d4) tgas(i,j,k)=1.0d4
      ...
      else if (iH2co .eq. 3) then
      gaHI   (i,j) = gaHIa   (indixe(i,j)) +  (logtem(i,j)  -  t1(i,j)) &
            *(gaHIa   (indixe(i,j)+1)-gaHIa   (indixe(i,j)))/tdef(i,j)
      gaH2   (i,j) = gaH2a   (indixe(i,j)) +  (logtem(i,j)  -  t1(i,j)) &
            *(gaH2a   (indixe(i,j)+1)-gaH2a   (indixe(i,j)))/tdef(i,j)
      gaHe   (i,j) = gaHea   (indixe(i,j)) +  (logtem(i,j)  -  t1(i,j)) &
            *(gaHea   (indixe(i,j)+1)-gaHea   (indixe(i,j)))/tdef(i,j)
      gaHp   (i,j) = gaHpa   (indixe(i,j)) +  (logtem(i,j)  -  t1(i,j)) &
            *(gaHpa   (indixe(i,j)+1)-gaHpa   (indixe(i,j)))/tdef(i,j)
      gael   (i,j) = gaela   (indixe(i,j)) +  (logtem(i,j)  -  t1(i,j)) &
            *(gaela   (indixe(i,j)+1)-gaHIa   (indixe(i,j)))/tdef(i,j)
      endif
#endif /* H2 */
#ifdef HD
...
!     Cooling Functions:
      edot(i,j,k)=(
#ifdef H &
       - ceHI   (i,j)* abun(i,j,k,1)*abun(i,j,k,3)            ! coll excit of HI &
       - ciHI   (i,j)* abun(i,j,k,1)*abun(i,j,k,3)            ! coll ioniz of HI &
       - reHII  (i,j)* abun(i,j,k,2)*abun(i,j,k,3)            ! recombin   of HII &
       - DM     (i,j)* abun(i,j,k,1)*abun(i,j,k,1)            ! DM ISM cooling
#endif /* H */
#ifdef He &
       - ceHeI  (i,j)* abun(i,j,k,4)*abun(i,j,k,3)**2*mhi*qrt ! coll excit of HeI &
       - ceHeII (i,j)* abun(i,j,k,5)*abun(i,j,k,3)*qrt        ! coll excit of HeII &
       - ciHeI  (i,j)* abun(i,j,k,4)*abun(i,j,k,3)*qrt        ! coll ioniz of HeI &
       - ciHeII (i,j)* abun(i,j,k,5)*abun(i,j,k,3)*qrt        ! coll ioniz of HeII &
       - ciHeIS (i,j)* abun(i,j,k,5)*abun(i,j,k,3)**2*mhi*qrt ! coll ioniz of HeIS &
       - reHeII1(i,j)* abun(i,j,k,5)*abun(i,j,k,3)*qrt        ! recombin   of HeII &
       - reHeII2(i,j)* abun(i,j,k,5)*abun(i,j,k,3)*qrt        ! recombin   of HeII &
       - reHeIII(i,j)* abun(i,j,k,6)*abun(i,j,k,3)*qrt        ! recombin   of HeIII
#endif /* He */ &
       - brem   (i,j)*(
#ifdef H &
                       abun(i,j,k,2)                          ! elec brems cooling
#endif /* H */
#ifdef He &
                     + abun(i,j,k,5)*qrt+abun(i,j,k,6)*qrt 
#endif /* He */ &
                    )* abun(i,j,k,3) &
       - cmpt   (i,j)* abun(i,j,k,3)*mh                       ! compton cooling &
                  )*mhi**2
!      if (tgas(i,j,k) .ge. 8500. .and. tgas(i,j,k) .le. 40000.)
!     .    edot(i,j,k) = tiny
!      tlambda(i)= tlambda(i) + dabs(edot(i,j,k))            ! cooling timescale
#ifdef H2
      if (iH2co .eq. 1) then                                 ! Lepp & Shull H2 cooling
      qq        = 1.2*(abun(i,j,k,1) * mhi      )**0.77 &
                +     (abun(i,j,k,8) * mhi * haf)**0.77
      vibl      =     (abun(i,j,k,1) * hyd01k(i,j) &
                +      abun(i,j,k,8) * h2k01 (i,j)*haf) &
                * mhi*8.18e-13
      edot(i,j,k) = edot(i,j,k) - &
               ( abun(i,j,k,8)*(vibh(i,j)/(1.+vibh(i,j)/vibl) &
               + roth(i,j)/(1.+roth(i,j)/(qq*rotl(i,j))))*mhi*haf)
      else if (iH2co .eq. 2) then                            ! Galli & Palla H2 cooling
      gphdl2      = gphdl(i,j  ) / (abun(i,j,k,1) * mhi)
      edot(i,j,k) = edot(i,j,k)  -  abun(i,j,k,8) * gphdl(i,j  ) &
                  / (1.0 + gphdl2 / gpldl(i,j)) &
                  * haf * mhi
      else if (iH2co .eq. 3) then                            ! Glover 2008 multichannel
                                                             ! H2 cooling
      galdl(i,j)  = gaHI(i,j) * abun(i,j,k,1) + gaH2(i,j)* abun(i,j,k,8) &
                  + gaHe(i,j) * abun(i,j,k,4) + gaHp(i,j)* abun(i,j,k,2) &
                  + gael(i,j) * abun(i,j,k,3)
      edot(i,j,k) = edot(i,j,k) - galdl(i,j)
      endif
#endif /* H2 */
#ifdef HD
      if (iHDco .eq. 1) then
       hdlte1      = hdlte(i,j)/(abun(i,j,k,12)*haf*mhi)
       hdlow1      = max(hdlow(i,j), tiny)
       edot(i,j,k) = edot(i,j,k) - abun(i,j,k,12) * &
...
#ifdef H &
                    + piHI  (i,j) * abun(i,j,k,1)        ! pi of HI
#endif /* H */
#ifdef He &
                    + piHeI (i,j) * abun(i,j,k,4) * qrt  ! pi of HeI &
                    + piHeII(i,j) * abun(i,j,k,5) * qrt  ! pi of HeII
#endif /* He */ &
                                                  ) * mhi
...
!DIR$ UNROLL UNROLL_I
          do i = is, ie
            dedot(i,j) = (
#ifdef H &
                    k1 (i,j) * abun(i,j,k,1) * abun(i,j,k,3)
#endif /* H  */
#ifdef He &
                  + k3 (i,j) * abun(i,j,k,4) * abun(i,j,k,3)*qrt &
                  + k5 (i,j) * abun(i,j,k,5) * abun(i,j,k,3)*qrt
#endif /* He */
#ifdef H2 &
                  + k8 (i,j) * abun(i,j,k,7) * abun(i,j,k,3) &
                  + k15(i,j) * abun(i,j,k,7) * abun(i,j,k,3) &
                  + k17(i,j) * abun(i,j,k,7) * abun(i,j,k,3) &
                  + k14(i,j) * abun(i,j,k,7) * abun(i,j,k,3)
#endif /* H2 */ &
                  ) * mhi * mhi - (
#ifdef H &
                    k2 (i,j) * abun(i,j,k,2) * abun(i,j,k,3)
#endif /* H  */
#ifdef He &
                  + k4 (i,j) * abun(i,j,k,5) * abun(i,j,k,3)*qrt &
                  + k6 (i,j) * abun(i,j,k,6) * abun(i,j,k,3)*qrt
#endif /* He */
#ifdef H2 &
                  + k7 (i,j) * abun(i,j,k,1) * abun(i,j,k,3) &
                  + k18(i,j) * abun(i,j,k,9) * abun(i,j,k,3)*haf
#endif /* H2 */ &
                  ) * mhi * mhi
#ifdef RT &
                  + (
#ifdef H &
                      k24(i,j) * abun(i,j,k,1)
#endif /* H  */
#ifdef He &
                    + k25(i,j) * abun(i,j,k,5) * qrt &
                    + k26(i,j) * abun(i,j,k,4) * qrt
#endif /* He */ &
                    ) * mhi
#endif /* RT */
!     Compute the photoionization timescale for this zone
            if      (icycle .eq. 1) then
!              t_photo = dabs(0.1*de(i,j,k,3)/dedot(i,j)*mhi)
              t_photo = dabs(0.1*(abun(i,j,k,3) + 1.0d-3*d(i,j,k)) &
                                /dedot(i,j)*mhi)
            else if (icycle .eq. 2) then
              t_photo = dabs(0.1*(abun(i,j,k,3) + 1.0d-3*d(i,j,k)) &
                                /dedot(i,j)*mhi)
!               t_photo = 0.1 * dmin1((abun(i,j,k,3) + 1.0d-3*d(i,j,k))/
!     .                               (dedot(i,j)*mhi),
!     .                               (abun(i,j,k,1) + 1.0d-3*d(i,j,k))/      
!     .                               (dedot(i,j)*mhi))
!              t_photo = dmax1(dabs(1.0d-06*abun(i,j,k,1)/dedot(i,j)*mhi),
!     .                        dabs(0.1*abun(i,j,k,3)/dedot(i,j)*mhi))
!              t_photo = dmax1(dmin1(0.1*dx1a(i)/clight,
!     .                        dabs(0.1*abun(i,j,k,1)/dedot(i,j)*mhi)),
!     .                        dabs(0.1*abun(i,j,k,3)/dedot(i,j)*mhi))
            endif
...
!DIR$ UNROLL UNROLL_I
          do i = is, ie
!        if (iter .eq. 70) print*,"computing temperature", i, tgas(i,j,k)
          tgas(i,j,k) = gamm1 * dabs(e(i,j,k)) * (tevk/cnorm) / (
#ifdef H &
         abun(i,j,k,1)       + abun(i,j,k,2)       + abun(i,j,k,3)
#endif /* H */
...
!            if (tgas(i,j,k) .gt. 1.0d4) tgas(i,j,k)=1.0d4
!        if (iter .eq. 70) print*,"computed temperature", i, tgas(i,j,k)
!        if (iter.eq.70 .and. i.eq.4) print*,abun(i,j,k,4),abun(i,j,k,6)
            logtem(i,j) = dlog(tgas(i,j,k))
            if (logtem(i,j) .lt. logtem0) logtem(i,j) = logtem0
            if (logtem(i,j) .ge. logtem9) logtem(i,j) = logtem9
            indixe(i,j) = min0(nratec-1, &
...
!     .    edot(i,j,k) = tiny
      ...
      gphdl2      = gphdl(i,j  ) / (abun(i,j,k,1) * mhi)
      edot(i,j,k) = edot (i,j,k) -  abun(i,j,k,8) * &
                     gphdl(i,j  ) / (1.0 + gphdl2 / gpldl(i,j)) &
                   * haf * mhi
      else if (iH2co .eq. 3) then                             ! Glover 2008 multichannel
                                                              ! H2 cooling
      galdl(i,j)  = gaHI(i,j) * abun(i,j,k,1) + gaH2(i,j)* abun(i,j,k,8) &
...
!     Compute the photoionization timescale for this zone
            if      (icycle .eq. 1) then
!              t_photo = dabs(0.1*abun(i,j,k,3)/dedot(i,j)*mhi)
              t_photo = dabs(0.1*(abun(i,j,k,3) + 1.0d-3*d(i,j,k)) &
                                /dedot(i,j)*mhi)
            else if (icycle .eq. 2) then
              t_photo = dabs(0.1*(abun(i,j,k,3) + 1.0d-3*d(i,j,k)) &
...
!DIR$ UNROLL UNROLL_I
          ...
          k56(i,j) = k56a(indixe(i,j))   + (logtem    (i,j) - t1  (i,j)) &
            ...
            ceHItot   = ceHItot &
                      + ceHI  (i,j)* abun(i,j,k,1)*abun(i,j,k,3) &
                      * mhi**2 * dtimin &
                      * 4. * pi * thd * (x1a(i+1)**3 - x1a(i)**3) 
            ciHItot   = ciHItot &
                      + ciHI  (i,j)* abun(i,j,k,1)*abun(i,j,k,3) &
                      * mhi**2 * dtimin &
                      * 4. * pi * thd * (x1a(i+1)**3 - x1a(i)**3) 
            ceHeItot  = ceHeItot &
                      + ceHeI (i,j)* abun(i,j,k,4)*abun(i,j,k,3)**2*qrt &
                      * mhi**3 * dtimin &
                      * 4. * pi * thd * (x1a(i+1)**3 - x1a(i)**3) 
            ceHeIItot = ceHeIItot &
                      + ceHeII(i,j)* abun(i,j,k,5)*abun(i,j,k,3)   *qrt &
                      * mhi**2 * dtimin &
                      * 4. * pi * thd * (x1a(i+1)**3 - x1a(i)**3) 
            ciHeItot  = ciHeItot &
                      + ciHeI (i,j)* abun(i,j,k,4)*abun(i,j,k,3)   *qrt &
                      * mhi**2 * dtimin &
                      * 4. * pi * thd * (x1a(i+1)**3 - x1a(i)**3) 
            ciHeIItot = ciHeIItot &
                      + ciHeII(i,j)* abun(i,j,k,5)*abun(i,j,k,3)   *qrt &
                      * mhi**2 * dtimin &
                      + ciHeIS(i,j)* abun(i,j,k,5)*abun(i,j,k,3)**2*qrt &
                      * mhi**3 * dtimin &
                      * 4. * pi * thd * (x1a(i+1)**3 - x1a(i)**3) 
            bremtot   = bremtot &
                      + brem  (i,j)*(abun(i,j,k,2)+abun(i,j,k,5)   *qrt &
                      +              abun(i,j,k,6)*qrt) * abun(i,j,k,3) &
                      * mhi**2 * dtimin &
                      * 4. * pi * thd * (x1a(i+1)**3 - x1a(i)**3) 
            ICtot     = ICtot &
                      + cmpt  (i,j)* abun(i,j,k,3) &
                      * mhi    * dtimin &
                      * 4. * pi * thd * (x1a(i+1)**3 - x1a(i)**3) 
            dv_ioniz(i,j,k) = dv_ioniz(i,j,k) + (
#ifdef H &
                              k24mv(i,j) * abun(i,j,k,1)
#endif /* H  */
#ifdef He &
                            + k26mv(i,j) * abun(i,j,k,4) * qrt &
                            + k25mv(i,j) * abun(i,j,k,5) * qrt
#endif /* He */ &
                            ) / d(i,j,k) * dtimin * mhi  * ci
!            rad_press(i,j,k) = rad_press(i,j,k) +
!     .                         f(i,j,1) * sigma24(1)*0.002*evhz/clight*
!     .                         0.5 * (abun(i-1,j,k,1) + abun(i,j,k,1)) * mhi *
!     .                         dtimin
#ifdef H
            scoef  =   (k2 (i,j) * abun(i,j,k,2) * abun(i,j,k,3) 
#ifdef H2 &
                   +    k13(i,j) * abun(i,j,k,1) * abun(i,j,k,8) &
                   +    k11(i,j) * abun(i,j,k,2) * abun(i,j,k,8)*haf &
                   +    k12(i,j) * abun(i,j,k,3) * abun(i,j,k,8) &
                   +    k14(i,j) * abun(i,j,k,7) * abun(i,j,k,3) &
                   +    k15(i,j) * abun(i,j,k,7) * abun(i,j,k,1) &
                   + 2.*k16(i,j) * abun(i,j,k,7) * abun(i,j,k,2) &
                   +    k18(i,j) * abun(i,j,k,9) * abun(i,j,k,3) &
                   +    k19(i,j) * abun(i,j,k,9) * abun(i,j,k,7)*haf 
#endif /* H2 */ &
                   ) *  mhi 
#if defined RT && defined H2 &
                   +    k31(i,j) * abun(i,j,k,8)             
#endif /* RT && H2 */
            acoef  =   (k1 (i,j) * abun(i,j,k,3) 
#ifdef H2 &
                   +    k7 (i,j) * abun(i,j,k,3) &
                   +    k8 (i,j) * abun(i,j,k,7) &
                   +    k9 (i,j) * abun(i,j,k,2) &
                   +    k10(i,j) * abun(i,j,k,9)    * haf &
                   + 2.*k22(i,j) * abun(i,j,k,1)**2 * mhi &
                   + 2.*R        * abun(i,j,k,1)
#endif /* H2 */ &
                   ) *  mhi 
#ifdef RT &
                   +    k24(i,j)
#endif /* RT */
            HIp    =   (scoef*dtimin + abun(i,j,k,1)) &
                   /   (1. + acoef*dtimin )
            scoef  =   (k1 (i,j) * abun(i,j,k,3) * HIp   
#ifdef H2 &
                   +    k10(i,j) * abun(i,j,k,9) * HIp * haf
#endif /* H2 */ &
                   ) *  mhi 
#ifdef RT &
                   +    k24(i,j) * HIp
#endif /* RT */
            acoef  =   (k2 (i,j) * abun(i,j,k,3) 
#ifdef H2 &
                   +    k9 (i,j) * HIp &
                   +    k11(i,j) * abun(i,j,k,8) * haf &
                   +    k16(i,j) * abun(i,j,k,7) &
                   +    k17(i,j) * abun(i,j,k,7) 
#endif /* H2 */ &
                   ) *  mhi 
            HIIp   =   (scoef*dtimin + abun(i,j,k,2)) &
                   /   (1. + acoef*dtimin) 
            abun(i,j,k,1) = HIp
            abun(i,j,k,2) = HIIp
#endif /* H */
#ifdef He
            scoef  =    k4(i,j)*abun(i,j,k,5)*abun(i,j,k,3) * mhi
            acoef  =    k3(i,j)*abun(i,j,k,3)*mhi
#ifdef RT &
                   +    k26(i,j)
#endif /* RT */
            HeIp   =   (scoef*dtimin + abun(i,j,k,4)) &
                   /   (1. + acoef*dtimin) 
            scoef  =   (k3(i,j) * HeIp         * abun(i,j,k,3) &
                   +    k6(i,j) * abun(i,j,k,6) * abun(i,j,k,3) &
                   ) *  mhi
#ifdef RT &
                   +    k26(i,j) * HeIp
#endif /* RT */
            acoef  =  ((k4(i,j) + k5(i,j))   * abun(i,j,k,3)) * mhi
#ifdef RT &
                   +    k25(i,j)
#endif /* RT */
            HeIIp  =   (scoef*dtimin + abun(i,j,k,5)) &
                   /   (1. + acoef*dtimin ) 
            scoef  =    k5 (i,j)*HeIIp*abun(i,j,k,3) * mhi
#ifdef RT &
                   +    k25(i,j)*HeIIp
#endif /* RT */
            acoef  =    k6 (i,j)*abun(i,j,k,3)*mhi
            HeIIIp =   (scoef*dtimin + abun(i,j,k,6)) &
                   /   (1. + acoef*dtimin) 
            abun(i,j,k,4) = HeIp
            abun(i,j,k,5) = HeIIp
            abun(i,j,k,6) = HeIIIp
            ...
            scoef  =  0.
#ifdef H2 &
                   +   (k8 (i,j) * abun(i,j,k,7) * HIp &
                   +    k15(i,j) * abun(i,j,k,7) * HIp &
                   +    k17(i,j) * abun(i,j,k,7) * HIIp &
                   ) *  mhi 
#endif /* H2 */
#ifdef RT
#ifdef H &
...
#endif /* He */
#ifdef H2 &
                  +     k14(i,j)*abun(i,j,k,7) &
                  -     k7 (i,j)*HIp &
                  -     k18(i,j)*abun(i,j,k,9)*haf
#endif /* H2 */ &
                  )  *  mhi
            dep   =    (scoef*dtimin + abun(i,j,k,3)) &
                  /    (1. + acoef*dtimin) 
            abun(i,j,k,3) = dep
#endif /* H or He */
!            if (tgas(is,j,k) .lt. 20000.) goto 666
            goto 666
#ifdef H2
            abun(i,j,k,7) = (k7(i,j)*abun(i,j,k,1)*abun(i,j,k,3)) &
                          /((k8(i,j) + k15(i,j))  *abun(i,j,k,1) &
             + ( k16(i,j)+k17(i,j))*abun(i,j,k,2)+k14(i,j)*abun(i,j,k,3)
#ifdef RT &
                 + k27(i,j)*mh
#endif /* RT */ &
                        )
            abun(i,j,k,9) = 2.*( k9 (i,j)*abun(i,j,k,1)*abun(i,j,k,2) &
                             + k11(i,j)*abun(i,j,k,8)*haf*abun(i,j,k,2) &
                             + k17(i,j)*abun(i,j,k,7)*abun(i,j,k,2)
#ifdef RT &
                 + k29(i,j)*abun(i,j,k,8)*mh
#endif /* RT */ &
                             ) &
                     / ( k10(i,j)*abun(i,j,k,1) + k18(i,j)*abun(i,j,k,3) &
                       + k19(i,j)*abun(i,j,k,7)
#ifdef RT &
                 + (k28(i,j)+k30(i,j))*mh
#endif /* RT */ &
                             )
            scoef = 2.*( k8 (i,j)*abun(i,j,k,7)*abun(i,j,k,1) &
                  + k10(i,j)*abun(i,j,k,9)*abun(i,j,k,1)*haf &
                  + k19(i,j)*abun(i,j,k,9)*abun(i,j,k,7)*haf &
                  + R       *abun(i,j,k,1)**2 &
                  + k22(i,j)*abun(i,j,k,1)**3 * mhi ) * mhi          
            acoef = ( k13(i,j)*abun(i,j,k,1) + k11(i,j)*abun(i,j,k,2) &
                    + k12(i,j)*abun(i,j,k,3) )*mhi
#ifdef RT &
                    + k29(i,j) + k31(i,j)
#endif /* RT */
            abun(i,j,k,8) = ( scoef*dtimin + abun(i,j,k,8)) &
                       / ( 1. + acoef*dtimin ) 
#endif /* H2 */
#ifdef HD
            scoef = (   k2 (i,j) * abun(i,j,k,11) * abun(i,j,k,3) &
                  +     k51(i,j) * abun(i,j,k,11) * abun(i,j,k,1) &
                  + 2.* k55(i,j) * abun(i,j,k,12) * abun(i,j,k,1) * thd &
                  ) * mhi 
            acoef      (k1 (i,j) * abun(i,j,k,3) &
                  +     k50(i,j) * abun(i,j,k,2) &
                  +     k54(i,j) * abun(i,j,k,8)    * haf &
                  +     k56(i,j) * abun(i,j,k,7) &
                  +     k22(i,j) * abun(i,j,k,1)**2 * mhi * haf &
                  +     k57(i,j) * abun(i,j,k,1)*abun(i,j,k,8)*mhi* haf &
                  ) * mhi 
#ifdef RT &
                  +     k24(i,j)
#endif /* RT */
            DIp   = ( scoef*dtimin + abun(i,j,k,10) ) &
                    ( 1. + acoef*dtimin )
            scoef =    (k1 (i,j) * DIp           * abun(i,j,k,3) &
                  +     k50(i,j) * abun(i,j,k,2) * DIp &
                  + 2.* k53(i,j) * abun(i,j,k,2) * abun(i,j,k,12) * thd &
                  ) * mhi
#ifdef RT &
                  +     k24(i,j) * DIp
#endif /* RT */
            acoef =    (k2 (i,j) * abun(i,j,k,3) &
                  +     k51(i,j) * abun(i,j,k,1) &
                  +     k52(i,j) * abun(i,j,k,8) * haf &
                  ) * mhi
            DIIp  = ( scoef*dtimin + abun(i,j,k,11) ) &
                  / ( 1. + acoef*dtimin )
            scoef = 3.*(k52(i,j) * DIIp * abun(i,j,k,8) * qrt &
                  +     k54(i,j) * DIp  * abun(i,j,k,8) * qrt &
                  +     k56(i,j) * DIp  * abun(i,j,k,7) &
                  +     k22(i,j) * DIp*abun(i,j,k,1)**2  * mhi * qrt &
                  +     k57(i,j)*DIp*abun(i,j,k,1)*abun(i,j,k,8)*mhi*qrt &
                       ) * mhi
            acoef =    (k53(i,j) * abun(i,j,k,2) &
                  +     k55(i,j) * abun(i,j,k,1) &
                       ) * mhi
!
            HDIp  = ( scoef*dtimin + abun(i,j,k,12) ) &
                  / ( 1. + acoef*dtimin )
            abun(i,j,k,10) = DIp
            abun(i,j,k,11) = DIIp
            abun(i,j,k,12) = HDIp
...
1000  continue
      call mconsrv
!     Convert the mass densities back into abundances.
      do k = ks, ke
       do j = js, je
        do i = is, ie
         do n  = 1, nspec
