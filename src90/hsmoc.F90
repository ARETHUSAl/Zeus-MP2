       ...
       do 50 j=js,je+1
         do 40 i=is,ie
           ...
           kp1=k+kone
             vave (k) = emf1(i,j,k)
             bave (k) = emf2(i,j,k)
!            vave (k) = 0.5 * ( v3(i,j,k) + v3(i,jm1,k) ) - vg3(k)
!            bave (k) = 0.5 * ( b3(i,j,k) + b3(i,jm1,k) )
             absb     = abs ( bave(k) )
             aave(k)  = 0.5 * absb * ( srd2(i,j,k) + srd2(i,j,km1) ) &
                                   / ( srd2(i,j,k) * srd2(i,j,km1) )
             sgnp     = sign ( haf, vave(k) - aave(k) )
             sgnm     = sign ( haf, vave(k) + aave(k) )
             srdp (k) = ( 0.5 + sgnp ) * srd2(i,j,km1) &
                      + ( 0.5 - sgnp ) * srd2(i,j,k  )
             srdm (k) = ( 0.5 + sgnm ) * srd2(i,j,km1) &
                      + ( 0.5 - sgnm ) * srd2(i,j,k  )
             srdpi(k) = 1.0 / srdp(k)
             srdmi(k) = 1.0 / srdm(k)
...
10         continue
!
!      Interpolate 1-D vectors of "v2" and "b2" in the 3-direction to
!  the footpoints of both characteristics.
!
           do 20 k=ks-2*kone,ke+2*kone
           km1=k-kone
           kp1=k+kone    !asif
             vtmp(k) = v2(i,j,k) - vg2(j)
             btmp(k) = b2(i,j,k)
20         continue
!          call x3zc1d ( vtmp, vchp, vchm, iords2, istps2, i, j
!    1                 , g31b, g31bi, g32a, g32ai, vpch, vmch )
!          call x3zc1d ( btmp, vchp, vchm, iordb2, istpb2, i, j
!    1                 , g31b, g31bi, g32a, g32ai, bpch, bmch )
!  1.  Evaluate monotonised, van Leer difference in "q" across the zone.
!
           do 820 k=ks-kone,ke+kone
           km1=k-kone
           kp1=k+kone
...
!  
             xi     = vchp(k) * fact
             q3     = sign ( haf, xi )
             vpch(k)= ( 0.5 + q3 ) * ( qv1 - xi * dv (km1) ) &
                    + ( 0.5 - q3 ) * ( qv2 - xi * dv (k  ) )
             bpch(k)= ( 0.5 + q3 ) * ( qb1 - xi * db (km1) ) &
                    + ( 0.5 - q3 ) * ( qb2 - xi * db (k  ) )
!
             xi     = vchm(k) * fact
             q3     = sign ( haf, xi )
             vmch(k)= ( 0.5 + q3 ) * ( qv1 - xi * dv (km1) ) &
                    + ( 0.5 - q3 ) * ( qv2 - xi * dv (k  ) )
             bmch(k)= ( 0.5 + q3 ) * ( qb1 - xi * db (km1) ) &
                    + ( 0.5 - q3 ) * ( qb2 - xi * db (k  ) )
 830       continue
!
!
...
60         continue
!
!      Interpolate 1-D vectors of "v3" and "b3" in the 2-direction to
!  the footpoints of both characteristics.
!
           do 70 j=js-2,je+2
             ...
             btmp(j) = b3(i,j,k)
70         continue
!           call x2zc1d ( vtmp, vchp, vchm, iords3, istps3, k, i
!     1                 , g2b, g2bi, vpch, vmch )
!           call x2zc1d ( btmp, vchp, vchm, iordb3, istpb3, k, i
!     1                 , g2b, g2bi, bpch, bmch )
!
           do 870 j=js-1,je+1
...
!  since dg32/dx2 .ne. 0.  Compute the two terms in "emf1".
!
           q1 = dt * g2bi(i)
           do 80 j=js,je+1
             q2           = sign ( one, bave(j) )
         ...
         do 220 i=is,ie+1
           do 190 k=ks,ke+kone
            ...
            kp1=k+kone
             vave (k) = v3inti(k,i)
             bave (k) = b3inti(k,i)
!            vave (k) = 0.5 * ( v3(i,j,k) + v3(im1,j,k) ) - vg3(k)
!            bave (k) = 0.5 * ( b3(i,j,k) + b3(im1,j,k) )
             absb     = abs ( bave(k) )
             aave (k) = 0.5 * absb * ( srd1(i,j,k) + srd1(i,j,km1) ) &
                                   / ( srd1(i,j,k) * srd1(i,j,km1) )
             sgnp     = sign ( haf, vave(k) - aave(k) )
             sgnm     = sign ( haf, vave(k) + aave(k) )
             srdp (k) = ( 0.5 + sgnp ) * srd1(i,j,km1) &
                      + ( 0.5 - sgnp ) * srd1(i,j,k  )
             srdm (k) = ( 0.5 + sgnm ) * srd1(i,j,km1) &
                      + ( 0.5 - sgnm ) * srd1(i,j,k  )
             srdpi(k) = 1.0 / srdp(k)
             srdmi(k) = 1.0 / srdm(k)
             vchp (k) = vave(k) - absb * srdpi(k)
...
190        continue
!
!      Interpolate 1-D vectors of "v1" and "b1" in the 3-direction to
!  the footpoints of both characteristics.
!
           do 200 k=ks-2*kone,ke+2*kone
             vtmp(k) = v1(i,j,k) - vg1(i)
             btmp(k) = b1(i,j,k)
200        continue
!           call x3zc1d ( vtmp, vchp, vchm, iords1, istps1, i, j
!     1                 , g31a, g31ai, g32b, g32bi, vpch, vmch )
!           call x3zc1d ( btmp, vchp, vchm, iordb1, istpb1, i, j
!     1                 , g31a, g31ai, g32b, g32bi, bpch, bmch )
!  1.  Evaluate monotonised, van Leer difference in "q" across the zone.
!
           do 1000 k=ks-kone,ke+kone
            km1=k-kone
            kp1=k+kone
             dqm      = ( vtmp(k  ) - vtmp(km1) ) * dx3bi(k  )
             dqp      = ( vtmp(kp1) - vtmp(k  ) ) * dx3bi(kp1)
...
!
         do 300 i=is,ie+1
           do 270 j=js,je+1
             vave (j) = v2inti(i,j)
             bave (j) = b2inti(i,j)
!            vave (j) = 0.5 * ( v2(i,j,k) + v2(im1,j,k) ) - vg2(j)
!            bave (j) = 0.5 * ( b2(i,j,k) + b2(im1,j,k) )
             absb     = abs ( bave(j) )
             aave (j) = 0.5 * absb * ( srd1(i,j,k) + srd1(i,j-1,k) ) &
                                   / ( srd1(i,j,k) * srd1(i,j-1,k) )
             sgnp     = sign ( haf, vave(j) - aave(j) )
             sgnm     = sign ( haf, vave(j) + aave(j) )
...
270        continue
!
!      Interpolate 1-D vectors of "v1" and "b1" in the 2-direction to
!  the footpoints of both characteristics.
!
           do 280 j=js-2,je+2
             ...
             btmp(j) = b1(i,j,k)
280        continue
!           call x2zc1d ( vtmp, vchp, vchm, iords1, istps1, k, i
!     1                 , g2a, g2ai, vpch, vmch )
!           call x2zc1d ( btmp, vchp, vchm, iordb1, istpb1, k, i
!     1                 , g2a, g2ai, bpch, bmch )
!
           do 1080 j=js-1,je+1
             dqm      = ( vtmp(j  ) - vtmp(j-1) ) * dx2bi(j  )
             dqp      = ( vtmp(j+1) - vtmp(j  ) ) * dx2bi(j+1)
